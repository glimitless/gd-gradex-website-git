// Source data for each tile in the archive grid.
const sampleItems = [
    {
...
function populateGrid(items) {
    ...
    // Build one card per artist/work entry.
    items.forEach((item) => {
    ...
    // After injecting images, assign each one a unique "cut" silhouette.
...
function createCircularCutClipPath() {
    // Keep point count fixed so CSS can smoothly tween to the hover shape.
    const points = 24;
...
function createFullSquareClipPath(points) {
    ...
    // Generate a square polygon with the same number of points.
    for (let i = 0; i < points; i++) {
        const angle = (Math.PI * 2 * i) / points;
        const dx = Math.cos(angle);
        const dy = Math.sin(angle);
        // Project each direction vector out to the square boundary.
...
function applyRandomCutShapes() {
    ...
    thumbnails.forEach((thumbnail) => {
        ...
        // CSS reads these variables for default and hover clip-path states.
